...
def main():
    ...
    if args.dataset=='ADL':
        model.load_state_dict(
            torch.load(f'{args.exp_path}/ADL/ambiguity/ckpts/model_epoch_90.pth', map_location='cpu')[
...
def eval(test_dataloader, model, criterion, epoch, illustration):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_dataloader):
            ...
            output, decoder_feature = model(frame)
            for i in range(64):
                feature=torch.sigmoid(decoder_feature[0,i,:,:])*255
                feature_numpy=feature.numpy()
                saved_path = f'{args.exp_path}/{args.dataset}/ambiguity/features/{i}.jpg'
            ...
            if illustration:
                for i in range(mask.shape[0]):
                    ...
                    saved_path = f'{args.exp_path}/{args.dataset}/ambiguity/features/original.jpg'
                    # print(saved_path)
