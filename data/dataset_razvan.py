...
def make_sequence_dataset(mode='train',dataset_name='ADL'):
    ...
    for par_video_id in sorted(par_video_id_list):
        video_id=par_video_id[3:]
        anno_name = video_id + '_nao.csv'
        anno_path = os.path.join(args.data_path, 'nao_annotations_razvan_filtered', anno_name)
        if os.path.exists(anno_path):
            ...
            if dataset_name=='ADL':
                ...
                img_path=os.path.join(args.data_path,'rgb_frames_razvan',video_id[0:3],video_id[:])
                # print(img_path)
...
def ini_datasets(dataset_name='ADL',original_split=False):
    ...
    return NAODatasetR(mode='train', data=train_data), NAODatasetR(mode='val', data=val_data)


if __name__ == '__main__':

    data=make_sequence_dataset(mode='train',dataset_name='EPIC')
    print(data.shape)
    data=make_sequence_dataset(mode='test',dataset_name='EPIC')
    print(data.shape)
